...
namespace BikeRace
{
    ...
    {
        //This Bike race Code developed by Gurpreet Singh
        //Creating the objects of classes
        Greyhound[] bikes = new Greyhound[4];//instance of greyhound class
        ...
        private void RaceTrackSetting()//this funtion is for setting the race track
        {
            ...
            bikes[1] = new Greyhound()
            {
                MyPictureBox = Racer2,
                RacetrackLength = raceTrackLength,
                StartingPosition = startingPosition
            };
            bikes[2] = new Greyhound()
            {
                MyPictureBox = Racer3,
                RacetrackLength = raceTrackLength,
                StartingPosition = startingPosition
            };
            bikes[3] = new Greyhound()
            {
                MyPictureBox = Racer4,
                RacetrackLength = raceTrackLength,
                StartingPosition = startingPosition
        ...
        private void BikeRacingStart()//this is function for starting the race
        {
            ...
            while (NoWinner)
            {
                ...
                for (int i = 0; i < bikes.Length; i++)//loop start and it will continue whenever length of greyhound class or track is not finished
                {
                    Thread.Sleep(1);//sleep function for the speed of cars
                    if (bikes[i].Run())//here run function is called from greyhound class for running the cars and condition is checked for three random cars
                    {
                        winningBike = i + 1;
                        NoWinner = false;
                        ...
                        foreach (Greyhound car in bikes)
                        {
